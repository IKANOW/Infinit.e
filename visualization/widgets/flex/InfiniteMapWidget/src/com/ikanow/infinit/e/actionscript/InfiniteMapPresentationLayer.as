...
package com.ikanow.infinit.e.actionscript
{
	...
	public class InfiniteMapPresentationLayer
	{
		...
		private var _guiLayer:InfiniteMapWidget = null;
		
		private var _modelLayer:InfiniteMapWidget = null;
		...
		private var _mapObj:Map = null;
		
		// Map settings:
		private var _currBounds:LatLngBounds = null;
		...
		private var _filteringEnabled:Boolean = true;
		 ...
		 */
		public function InfiniteMapPresentationLayer( guiLayer:InfiniteMapWidget, modelLayer:InfiniteMapWidget )
		{
			_guiLayer = guiLayer;
		...
		public function doMapDisplayProcessing():void
		{
			if ( ( null != _currBounds ) && ( -1 != _currZoom ) && ( null != _entityMarkers ) && ( null != _mapObj ) )
			{
				...
				clusterer.cluster( _mapObj, _entityMarkers, _currBounds.getNorth(), _currBounds.getSouth(), _currBounds.getEast(), _currBounds.getWest(), .1, _currBounds, _currZoom );
		...
		public function onMapSizeChanged_fromGUI( newBounds:LatLngBounds, newZoom:Number, mapObj:Map ):void
		{
			if ( null != newBounds )
			{
			...
			if ( _guiLayer.isHeatMapEnabled_fromPresentation() )
			{
				...
				for each ( var geoPoint:Object in _geoCounts )
				{
					if ( InfiniteMapUtils.allowOntologyType( geoPoint[ "type" ] ) )
					{
						...
						var p:Point = _mapObj.fromLatLngToViewport( ll, true );
		...
		private function findCluster( lat:Number, lon:Number ):ArrayCollection
		{
			...
			{
				if ( group[ 0 ].geotag.lat == lat && group[ 0 ].geotag.lon == lon )
				{
					return group;
				}
			}
